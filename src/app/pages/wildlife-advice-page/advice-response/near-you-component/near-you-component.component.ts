...
export class NearYouComponentComponent implements OnInit {
  ...
  private adviceThree: AdviceSave = {
    Header: "",
    Pathname: "",
    Justification: "",
    BodyText: "",
  ...
  public displayArray: AdviceSave[] = [
    this.adviceOne,
    this.adviceTwo,
    this.adviceThree,
  ];
  ...
  constructor(
    ...
    public allAnswersService: AllAnswers
  ) {
    this.ourAnswersService = this.allAnswersService
      .getAnswerUpdateListener()
      .subscribe((retrievedAnswers: CompleteAnswerSet) => {
      ...
      .subscribe(
        (response) => {
          ...
          for (let i = 0; i < response.length; i++) {
            ...
            for (
              let j = 0;
              j < ourUserData.properties.savedAdvice.length;
              j++
            ) {
              inspoArray[count] = {
                ...
                Justification:
                  ourUserData.properties.savedAdvice[j].Justification,
        ...
        (response) => {
          ...
          if (closeArray.length != 0) {
            for (let n = 0; n < closeArray.length; n++) {
              if (
                closeArray[n].Header == "Put in a Small Water Feature" ||
